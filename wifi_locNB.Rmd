...
####**Attribute Defitions**
```{r message=FALSE}
#-Prepare table of variable definitions
...
##4. Explore the Data
Now that we've cleaned up the data set we'll prepare some visualizations to help us understand the data.  The image below is a satellite picture of the 3-building complex where the Wi-Fi fingerprint data was obtained.  The get_map() and ggmap() functions were used to import and plot the image.

```{r message=FALSE, cache=TRUE}
#-Satellite image of buildings where data set produced
...
```{r cache=TRUE}
#3D plot of data- all buildings & floors
...
```{r cache=TRUE}
#-Barplot of instance counts across location IDs
...
knn_fit <- saveRDS(knn_fit, 'knn_fit.rds')     #-save model
```
...
dtree_fit <- saveRDS(dtree_fit, 'dtree_fit.rds')     #-save model
```
...
rf_fit <- saveRDS(rf_fit, 'rf_fit.rds')     #-save model
```
####**Assess RF Model**
...
rf_hit <- wifi_trainData[which(rf_fit$finalModel$predictions==wifi_train$ID),]  #-correctly classified

rf_hit$ID <- factor(rf_hit$ID)
nlevels(rf_hit$ID) #726 levels

rf_mis <- wifi_trainData[which(rf_fit$finalModel$predictions != wifi_train$ID),]  #-incorrectly classified

rf_mis$ID <- factor(rf_mis$ID)
nlevels(rf_mis$ID)  #598 levels

```
We can look at the distribution of instances for correctly and incorrectly classified instances.

```{r}

#-get counts of instances correctly and incorrectly classified
...
ggplot(ID_hitFreq, aes(x=Freq, fill='Correct')) + geom_histogram(binwidth = 2) +
  geom_histogram(data=ID_misFreq, aes(x=Freq, fill='Incorrect'), alpha=0.4, binwidth=2)+
...
```
Clearly, locations with more instances in the data set were more likely to be correctly classified.
...
knitr::kable(phoneID_table) %>% 
  kable_styling(bootstrap_options = c('striped', 'hover', 'responsive', 'bordered'), 
                full_width = F, 
                font_size = 14) %>% 
  row_spec(18, bold=T, background = 'yellow')
```
We showed earlier that the number of instances per location was positively correlated with classification accuracy.  To see if that is what is driving the poor accuracy with 
...
ggplot(phone_misFreq, aes(x=Freq)) + geom_histogram(binwidth=2,fill='blue') +
  ...
  theme(panel.border=element_rect(colour='black', fill=NA))
```
The number of instances does not seem to be driving the poor accuracy obtained with PhoneID 17.  This phone does seem to be a legitamite outlier so we will remove its contribution to the data set and retrain the Random Forest model.




```{r}
#-Histogram and WAP counts of misclassified locations by phoneID 17
hist(phone_mis17$WAP_num)
table(phone_mis17$WAP_num)
```
```{r}
