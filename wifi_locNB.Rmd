...
Next we will try a tree-based classifier and save the model withe the saveRDS() function.
...
ggplot(ID_hitFreq, aes(x=Freq, fill='Correct')) + geom_histogram() +
...
```
```{r}
ggplot(phone_misFreq, aes(x=Freq)) + geom_histogram(fill='blue')
```
